...
phylo_tree i2l::io::load_newick(const string& file_name)
{
    std::cout << "Loading newick: " + file_name << std::endl;

    /// Load a tree from file
    newick_parser parser;
    i2l::io::buffered_reader reader(file_name);
    if (reader.good())
    {
        while (!reader.empty())
        {
...
phylo_tree i2l::io::parse_newick(std::string_view newick_string)
{
    newick_parser parser;
...
std::string i2l::io::to_newick(const phylo_tree& tree, bool jplace)
{
