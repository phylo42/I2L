...
template <>
positioned_pkdb_value i2l::make_pkdb_value<positioned_phylo_kmer>(phylo_kmer::branch_type branch,
...
template <>
unpositioned_pkdb_value i2l::make_pkdb_value<unpositioned_phylo_kmer>(phylo_kmer::branch_type branch,
                                                                       phylo_kmer::score_type score,
                                                                       phylo_kmer::pos_type position)
{
    (void)position;
...
template <>
positioned_phylo_kmer i2l::make_phylo_kmer(phylo_kmer::key_type key, phylo_kmer::score_type score,
                                            phylo_kmer::pos_type position)
{
...
template <>
unpositioned_phylo_kmer i2l::make_phylo_kmer(phylo_kmer::key_type key, phylo_kmer::score_type score,
                                              phylo_kmer::pos_type position)
{
    (void)position;
    return { key, score };
...
bool i2l::operator==(const unpositioned_phylo_kmer& lhs, const unpositioned_phylo_kmer& rhs) noexcept
{
    if (lhs.is_nan() || rhs.is_nan())
    {
        return false;
...
phylo_kmer::score_type i2l::score_threshold(phylo_kmer::score_type omega, size_t kmer_size)
{
    return std::pow(omega / seq_traits::alphabet_size, phylo_kmer::score_type(kmer_size));
}

template<>
optional<no_ambiguity_policy::value_type> i2l::encode_kmer<no_ambiguity_policy>(std::string_view kmer)
{
...
template<>
optional<one_ambiguity_policy::value_type> i2l::encode_kmer<one_ambiguity_policy>(std::string_view kmer)
{
...
std::string i2l::decode_kmer(phylo_kmer::key_type key, size_t kmer_size)
{
