    ...
    bool> kmer_iterator<no_ambiguity_policy>::encode_kmer()
{
    ...
    else
    {
        ...
        if (auto code = i2l::encode_kmer<no_ambiguity_policy>(kmer_view); code)
        {
            key = *code;
            success = true;
        }
    ...
    bool> kmer_iterator<no_ambiguity_policy>::encode_kmer_from_previous() const
{
    ...
    if (position <= _sequence_view.size() - _kmer_size)
    {
        ...
        if (auto new_code = i2l::encode<no_ambiguity_policy>(new_base); new_code)
        {
    ...
    bool> kmer_iterator<one_ambiguity_policy>::encode_kmer()
{
    ...
    else
    {
        ...
        /// resolve ambiguities
        if (auto codes = i2l::encode_kmer<one_ambiguity_policy>(kmer_view); codes)
        {
            keys = *codes;
            success = true;
        }
