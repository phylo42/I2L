...
namespace i2l
{
    ...
    /// The "greater" comparator for batch loaders. Compares its top elements
    /// if they have any. If one element is empty, it is "less"
    ...
    inline void load_db(Archive& ar, Database& db, unsigned int version)
    {
        ...
        else
        {
            ...
            db.set_num_total_entries(header.num_entries);
            ...
            const auto max_pk_allowed = (size_t)std::min(user_mu * header.num_entries,
                                                         (float)db.get_num_max_entries());
